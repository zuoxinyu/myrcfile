...
"MAP{
    ...
    map <C-F12> :!ctags -R --c-kinds=+p --fields=+iaS --extra=+q .<CR><CR>	"自动生成ctags"
...
"UI{
    ...
    hi User8 ctermfg=black ctermbg=yellow
    hi User9 ctermfg=black ctermbg=white
    hi User0 ctermfg=yellow  ctermbg=138 
    "Statusline{                                            
    	set statusline=  
    	set statusline+=%7*\[%n]                                  "buffernr  
    	set statusline+=%1*\ %<%F\                                "文件路径  
    	set statusline+=%7*\ %y\                                  "文件类型  
    	set statusline+=%3*\ %{''.(&fenc!=''?&fenc:&enc).''}      "编码1  
    	set statusline+=%3*\ %{(&bomb?\",BOM\":\"\")}\            "编码2  
    	set statusline+=%8*\ %{&ff}\                              "文件系统(dos/unix..)   
    	set statusline+=%2*\ %{&spelllang}\%{HighlightSearch()}\  "语言 & 是否高亮，H表示高亮?  
    	...
    	set statusline+=%9*\ %=\ row:%l/%L\ (%03p%%)\             "光标所在行号/总行数 (百分比)  
    	set statusline+=%2*\ col:%03c\                            "光标所在列  
